...
- `SESSION_JSON_B64`: Base64-encoded contents of `data/session.json` (generated by `pnpm run session:init`). See the deployment guides for encoding instructions.
...
3. Run Terraform validation on pull requests that change `infra/**`

## 🛠️ Development

- `pnpm test`: Run unit and integration tests.
- `pnpm run lint`: Enforce code quality using ESLint 9.
- `pnpm run build`: Compile TypeScript to ESM.
- `pnpm run bundle`: Bundle the application into a single file using `esbuild` (includes both clipper and weekly summary).
- `pnpm run test:weekly-summary`: Test weekly summary functionality locally (see [Testing Guide](docs/testing-weekly-summary.md)).
